    ...
    static getHomeHtml(homeHtml , students) {
        ...
        students.map(async (student, index) => {
            tbody +=`
                    ...
                    <td><img src="./public/${student.image}" alt="Image" style="width: 100px;height: 100px;"></td>
                    <td>${student.nameScore}</td>
                    <td>${student.score}</td>
                    <td><a href="/product/edit/${student.id}"><button class="btn btn-primary">Edit</button></a></td>
                    <td>
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        <form action="/product/delete/${student.id}" method="post">
        ...
        if (req.method === 'GET') {
            fs.readFile('./views/create.html', 'utf-8', async (err, createHtml) => {
                if (err) {
                    ...
                    let scores = await ScoreService.getScore();
                    let optionHtml = ''
                    scores.map((score) => {
                        optionHtml += `<option value="${score.id}">${score.nameCategory}</option>`
                    })
            ...
            req.on('end', async err => {
                if (err) {
                    ...
                    let data = await StudentService.save(student);
        ...
        if (req.method === 'GET') {
            fs.readFile('./views/edit.html', 'utf-8', async (err, editHtml) => {
                if (err) {
                    ...
                    let categoryName = await StudentService.findCategoryNameByProductId(id);
                    // let optionHtml = '';
                    ...
                    editHtml = editHtml.replace('{price}', student[0].age);
                    editHtml = editHtml.replace('{description}', student[0].address);
                    editHtml = editHtml.replace('{idCategory}', student[0].idCategory);
