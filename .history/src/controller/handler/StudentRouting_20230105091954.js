        ...
        if (req.method === 'GET') {
            fs.readFile('./views/edit.html', 'utf-8', async (err, editHtml) => {
                if (err) {
                    ...
                    editHtml = editHtml.replace('{scoreTheory}', student[0].address);
                    editHtml = editHtml.replace('{evaluate}', student[0].idCategory);
                    editHtml = editHtml.replace('{scorePractice}', categoryName[0].nameCategory);
                    editHtml = editHtml.replace('{description}', optionHtml);
