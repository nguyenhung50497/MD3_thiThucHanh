...
class StudentService {
    ...
    static create(student) {
        let connect = connection.getConnection();
        return new Promise((resolve, reject) => {
            connect.query(`INSERT INTO studentmanager.students(name, class, scoreTheory, evaluate, scorePractice, description) VALUES ('${student.name}', '${+student.class}', ${student.address}', '${student.image}')`, (err, data) => {
                 if (err) {
                     reject(err);
                 }
                 else {
    ...
    static update(student, id) {
        let connect = connection.getConnection();
        return new Promise((resolve, reject) => {
            connect.query(`UPDATE studentmanager.students SET name = '${student.name}', class = '${student.class}', scoreTheory = ${student.scoreTheory}, scoreTheory = ${student.scoreTheory}, evaluate = ${student.evaluate}, scorePractice = ${student.scorePractice}, description = ${student.description} WHERE id = ${id}`, (err, students) => {
                 if (err) {
                     reject(err);
                 }
                 else {
