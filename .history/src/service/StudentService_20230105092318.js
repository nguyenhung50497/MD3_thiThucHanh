...
class StudentService {
    ...
    static update(student, id) {
        let connect = connection.getConnection();
        return new Promise((resolve, reject) => {
            connect.query(`UPDATE studentmanager.students SET name = '${student.name}', class = '${student.class}', scoreTheory = ${student.scoreTheory}, scoreTheory = ${student.scoreTheory}, evaluate = ${student.evaluate}, scorePractice = ${student.scorePractice}, description = ${student.description} WHERE id = ${id}`, (err, student) => {
                 if (err) {
                     reject(err);
                 }
                 else {
