...
class ProductService {
    ...
    static save(student) {
        let connect = connection.getConnection();
        return new Promise((resolve, reject) => {
            connect.query(`INSERT INTO my_database1.product(name, price, description, image, idCategory) VALUES ('${student.name}', ${student.price}, '${product.description}', '${product.image}', '${+product.idCategory}')`, (err, data) => {
                 if (err) {
                     reject(err);
                 }
                 else {
