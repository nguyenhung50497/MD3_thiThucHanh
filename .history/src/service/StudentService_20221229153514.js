...
class ProductService {
    ...
    static update(student, id) {
        let connect = connection.getConnection();
        return new Promise((resolve, reject) => {
            connect.query(`UPDATE my_database1.product SET name = '${student.name}', price = ${student.price}, description = '${student.description}', idCategory = '${product.idCategory}' WHERE id = ${id}`, (err, products) => {
                 if (err) {
                     reject(err);
                 }
                 else {
