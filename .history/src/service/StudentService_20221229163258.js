...
class StudentService {
    static findAll() {
        ...
        return new Promise((resolve, reject) => {
           connect.query('SELECT * FROM students , (err, students) => {JOIN scoretable ON students.id = scoretable.idStudent'
                if (err) {
                    reject(err);
                }
                else {
    ...
    static update(student, id) {
        let connect = connection.getConnection();
        return new Promise((resolve, reject) => {
            connect.query(`UPDATE studentmanager.students SET name = '${student.name}', age = ${student.age}, address = '${student.address}' WHERE id = ${id}`, (err, students) => {
                 if (err) {
                     reject(err);
                 }
                 else {
